...
double mixed_method(vector<pthread_t>& threads, FILE* output) {
    ...
    fprintf(output, "\nTotal time taken in mixed method: %f seconds\n", (mixed_time));
    fprintf(output, "Average execution time of bounded threads: %f seconds\n", avg_time_for_bounded_thread / bt);
...
int main() {
    ...
    fprintf(output, "Size of matrix: %d \nNumber of threads: %d \nBounded threads: %d \nNumber of cores: %d\nNumber of cores assigned to bounded threads: %d\n", n, k, bt, c, bc);
    ...
    double mixed_time = mixed_method(threads, output);

    // FILE* temp_out = fopen("output2.txt", "w");
    // fprintf(temp_out, "%f", mixed_time);
    // fclose(temp_out);

    // FILE *output3 = fopen("exp2boundedmixed.txt","w");
    // fprintf(output3, "%f\n", avg_time_for_bounded_thread / bt);
    // fclose(output3);

    // FILE *output4 = fopen("exp2unboundedmixed.txt","w");
